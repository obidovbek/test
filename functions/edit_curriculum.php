<?php
    ...
    function do_action(){
        ...
        $folder = "../data/timetables/".$api_data->name."/curriculum/".$api_data->courseNumber."/".$api_data->file_name;
        $curriculum = json_decode(file_get_contents($folder));
        $curriculum = $api_data->curriculum;
        if(file_put_contents($folder, json_encode($curriculum))){
        	$result->status = 200;
        }
    }
