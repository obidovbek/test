<?php
    ...
    function do_action(){
        ...
        $data = $api_data->data;
        $new_time_table = json_decode(file_get_contents($folder.$data->copyTimeTable->time.".json"));
        $new_time_table->name = $data->newName;
        $time = time();

        $myfile = fopen($folder.$time.".json", "w") or die("Unable to open file!");
        fwrite($myfile, json_encode($new_time_table));
        fclose($myfile);
        ...
        recurseCopy($folder.$data->copyTimeTable->time, $folder.$time);

        // $result->time_table = $new_time_table;
        $result->status = 200;
    }
