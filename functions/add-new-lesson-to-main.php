<?php
    ...
    function do_action(){
        ...
        if(empty($result->exists_teacher) && empty($result->exists_room)){
            save_to_db();
        }else{
            $result->status = 198;
        }
    ...
    function save_to_db(){
        ...
        $data = $api_data->data;
        $folder = "../data/timetables/".$api_data->name;
        ...
        $folder_teachers = $folder."/teachers/";
        ...
        $data->teacherAvailab->roomData = $data->roomData;
        $data->teacherAvailab->roomData->availab = "";
        array_push($teacher->availab, $data->teacherAvailab);

        $folder_room = $folder."/rooms/";
        ...
        $data->roomAvailab->teacherData = $data->teacherData;
        $data->roomAvailab->teacherData->availab = "";
        array_push($room->availab, $data->roomAvailab);

        if(file_put_contents($folder_teachers.$file_name_teachers, json_encode($teacher)) && 
