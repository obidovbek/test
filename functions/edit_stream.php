<?php
    ...
    function do_action(){
        ...
        // $stream_folder = $api_data->stream[0]->stream_folder;
        $folder = "./data/timetables/".$api_data->name."/streams/".$api_data->courseNumber."/".$api_data->stream_folder."/";
        $old_files=scandir($folder);
        if (!file_exists($folder)) { mkdir($folder, 0777, true); }

        foreach ($api_data->stream as $key_stream_item => $value_stream_item) {
            // $value_stream_item->stream_folder = $stream_folder;
        ...
        // $result->stream_folder = $stream_folder;
        // $remove_folder = "./data/timetables/".$api_data->name."/streams/".$api_data->courseNumber."/".$api_data->stream_folder."/";
        deleteFiles($folder, $old_files);    
        $result->status = 200;
    }
